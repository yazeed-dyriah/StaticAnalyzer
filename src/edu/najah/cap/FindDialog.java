	...
	private void find(String pattern) {
		if (!finishedFinding) {
			if (matcher.find()) {
				...
				editor.getTextPanel().moveCaretPosition(matcher.start());
				editor.getTextPanel().select(selectionStart, selectionEnd);
			} else {
			...
			matcher = Pattern.compile(pattern).matcher(editor.getTextPanel().getText());
